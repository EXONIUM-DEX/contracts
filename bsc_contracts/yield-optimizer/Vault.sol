...
contract Vault is ERC20Upgradeable, IVault, IUpgradeSource, ControllableInit, VaultStorage {
    ...
    function initializeVault(
      ...
      require(_toInvestDenominator != 0, "Vault: cannot divide by 0");
       ...
       __ERC20_init(
        string(abi.encodePacked("ec", ERC20Upgradeable(_underlying).symbol())), //ecCAKE_LP
        string(abi.encodePacked("ec", ERC20Upgradeable(_underlying).symbol()))  //ecCAKE_LP
      );
      //I still have finded this implement functionr yet
      // _setupDecimal(ERC20Upgradeable(_underlying).decimals());
  ...
  modifier defense() {
    require(
      ...
      "Vault:This smart contract has been grey listed"  // make sure that it is not on our greyList.
    );
    _;
  }
  ...
  */
  function doHardWork() whenStrategyDefined onlyControllerOrGovernance external override {
    ...
    //Transfer all token(underlying in Vault) to Strategy(follow to ratio numerator/denominator)
    invest();
    //Call doHardWork
  ...
  */
  ...
  function setStrategy(address _strategy) public override onlyControllerOrGovernance {
    require(canUpdateStrategy(_strategy),"Vault: The strategy exists and switch timelock did not elapse yet");
  ...
  */
  ...
  function withdraw(uint256 numberOfShares) override external {
    //numberOfShares is amount token of user need to withdraw
  ...
  */
  ...
  function deposit(uint256 _amount) external override defense {
